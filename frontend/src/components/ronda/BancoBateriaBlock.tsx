...
interface BancoBateriaBlockProps {
  item: { identificador: string };
  value: BancoBateriaData;
  onChange: (data: BancoBateriaData) => void;
  falha: boolean;
  observacao: string;
...
export function BancoBateriaBlock({
  item,
  value,
  onChange,
  falha,
  ...
  onFalhaChange,
  onObservacaoChange,
}: BancoBateriaBlockProps) {
  function set<K extends keyof BancoBateriaData>(key: K, val: BancoBateriaData[K]) {
  ...
  return (
    <div style={blockCardStyle(falha)}>
      ...
      <div style={fieldGroup}>
        <label style={label}>Autonomia (min)</label>
        <input
          type="number"
          step="1"
          min="0"
          value={value.autonomia_min}
          onChange={(e) => set('autonomia_min', e.target.value === '' ? '' : Number(e.target.value))}
          placeholder="Ex: 30"
          style={inputStyle}
        />
      </div>
      ...
      <div style={row2}>
        <div style={fieldGroup}>
          <label style={label}>Última manutenção</label>
          <input
            type="date"
            value={value.data_ultima_manutencao}
            onChange={(e) => set('data_ultima_manutencao', e.target.value)}
            style={inputStyle}
          />
        </div>
        <div style={fieldGroup}>
          <label style={label}>Próxima manutenção</label>
          <input
            type="date"
...
export default BancoBateriaBlock;
